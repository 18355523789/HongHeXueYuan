...
-- 更多示例題目
INSERT INTO quiz_question (course_id, question_text, question_type, correct_answer, explanation, difficulty_level, create_time, update_time) 
VALUES (1, '以下哪個是Vue.js的響應式數據綁定指令？', 'single', 'C', 'v-model指令用於雙向數據綁定，是Vue.js的核心特性之一', 1, NOW(), NOW());

SET @question_id = LAST_INSERT_ID();

INSERT INTO quiz_option (question_id, option_text, option_key, is_correct, create_time, update_time) VALUES
(@question_id, 'v-bind', 'A', 0, NOW(), NOW()),
(@question_id, 'v-on', 'B', 0, NOW(), NOW()),
(@question_id, 'v-model', 'C', 1, NOW(), NOW()),
(@question_id, 'v-if', 'D', 0, NOW(), NOW());

-- 最後一個判斷題
INSERT INTO quiz_question (course_id, question_text, question_type, correct_answer, explanation, difficulty_level, create_time, update_time) 
VALUES (1, 'MySQL是關係型數據庫管理系統', 'judge', 'true', 'MySQL確實是一個開源的關係型數據庫管理系統', 1, NOW(), NOW());
